...
import { CrawlingSession, CrawlingSessionDocument } from "../schemas/crawling-session.model";

@Injectable()
class CrawlingSessionRepo {
  constructor(@InjectModel(CrawlingSession.name) private crawlingSessionModel: Model<CrawlingSession>) { }

  public async create(crawlingSession: CrawlingSession): Promise<CrawlingSessionDocument> {
    try {
  ...
  public async updateOne(filterOption: RootFilterQuery<CrawlingSession>, webiste: Partial<CrawlingSession>): Promise<UpdateWriteOpResult> {
    try {
      Logger.log("executing: CrawlingSessionRepo -> update");

      const data = await this.crawlingSessionModel.updateOne(filterOption, webiste)
  ...
  public async findOne(params: RootFilterQuery<CrawlingSession>): Promise<CrawlingSessionDocument | null> {
    try {
  ...
  public async find(params: RootFilterQuery<CrawlingSession>): Promise<CrawlingSessionDocument[]> {
    try {
  ...
  public async delete(params:RootFilterQuery<CrawlingSession>): Promise<DeleteResult> {
    try {
      Logger.log("executing: CrawlingSessionRepo -> create")
